module Platform
  module ApplicationHelper
    ...
    def post_timestamp(post_id)
      Rails.cache.fetch(redis_timestamp_key_for(Post, post_id)) do
        "#{redis_timestamp_key_for(Post, post_id)}-#{Time.now.to_i}"
      end
    end

    def category_timestamp(category_id)
      Rails.cache.fetch(redis_timestamp_key_for(Category, category_id)) do
        "#{redis_timestamp_key_for(Category, category_id)}-#{Time.now.to_i}"
      end
    end

    def column_timestamp(column_id)
      Rails.cache.fetch(redis_timestamp_key_for(Column, column_id)) do
